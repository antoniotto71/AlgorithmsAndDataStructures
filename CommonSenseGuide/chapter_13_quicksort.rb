class SortableArray
  ...
  def partition!(left_pointer, right_pointer)
    ...
    while true
      ...
      # points to a value that is grater than the pivot
      while @array[right_pointer] > pivot
        right_pointer -= 1
      end

      # We have now reached the point where we've stopped
  ...
  def quickselect!(kth_lowest_value, left_index, right_index)
    ...
    # we know we've found the value we are looking for:
    if right_index - left_index <= 0
    ...
    # If what we're looking for is o the left of the pivot:
    if kth_lowest_value < pivot_index
      ...
      # If what we're looking for is o the right of the pivot:
...
array = [0,5,8,9,1,3,2,4,12,18,33,6]

sortable_array = SortableArray.new(array)

sortable_array.quicksort!(0, array.length - 1)

p sortable_array.quickselect!(9, 0, array.length - 1)
